...
function g1 = function_BlockTridiagonalInverse2(ell, maxPhase, lambda1, lambda2, mu1, mu2, gamma1, gamma2, c, m)
    %We calculate g1, which includes blocks g1j of matrix G  =A^-1  
    Msize = maxPhase + 1;    
    ...
    %calculate d11R
    CR = zeros(Msize, N*Msize);% We put c_iR,   i = N,...,2 in this matrix. NOTE: for convenience, we set the size of DR and CR to be (Msize, N*Msize) and not (Msize, (N-1)*Msize). Basically, we assume that c1R = d11R = 0. But we update d11R later.
    %DR = zeros(Msize, N*Msize);% We put d_iiR, i = N,...,2 in this matrix
