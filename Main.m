...
indicatorM = zeros(cutOffPoint+1,cutOffPoint+1); %This matirx is for debugging purposes only. At iteration k, the vector indicatorV associated with this iteration is put in column k of this matrix
startTime = tic;
k = 0;    
m_initial = 4;
...
med_logic = min(min(upper_R0 - lower_R0)) < - eps || any(any(lower_R0 < -eps)); % We use eps instead of 0 to account for the floating-point relative error
if med_logic
...
while  sum(lowerBoundSumV) < 1 - epsilon_aggrProb 
    ...
    if k < tau
        ...
        med_logic = min(min(upper_Rk - lower_Rk)) < - eps || any(any(lower_Rk < -eps)); 
        if med_logic
    ...
    lower_x_ellPrime_Matrix(k+1,:) = lower_xk; % We record the bounds for all x_ellPrime (unnormalized probs), but we will use only those that satisfy the torlerance error for performance measure calculation
    ...
    if (~isempty(firstZeroInd)  && firstZeroInd > 1 )   %sometimes, it is possible that, for example, pi for levels 3-10 is within the tolerence error, but pi for 0-2 is not. firstZeroInd gives the first level for which the tolerence level is NOT satisfied. We calculate the sum of probs for all levels below that level and check if the sum is larger than 1 - epsilon_aggrProb. If it is, then we go out of the while loop. 
        ...
        %there is at least one level that does not satisfy the error tolerance and
    ...
    if toc(startTime)>3600*hours
        lowerBoundSumV = 1; %it terminates the while loop
        tookTooLong = true(1);
    end
end
